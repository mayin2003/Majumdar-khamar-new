...
export const AdminProducts: React.FC = () => {
  ...
  // Form State (strictly NO price or forSale!)
  ...
  const demoCount = products.filter(p => p.isDemo === true || p.id.startsWith('prod-')).length;
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (formImageFiles.length >= 3) {
      setFormError('সর্বোচ্চ ৩টি ছবি যোগ করা যাবে।');
      return;
    }
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      setFormError('প্রোডাক্টের নাম পূরণ করা আবশ্যক!');
      return;
    }

    setIsSubmitting(true);
    try {
      const finalSlug = formData.slug.trim() || `prod-${Date.now()}`;
      const productToSave: Product = {
        ...
        gender: formData.gender.trim() || 'পুরুষ',
      ...
      setFormError('প্রোডাক্ট সংরক্ষণ করতে ব্যর্থ হয়েছে।');
    } finally {
  ...
  const handleDeleteDemoData = async () => {
    try {
      ...
      setDemoDeletedMsg(`${count} টি ডেমো প্রোডাক্ট সফলভাবে মুছে ফেলা হয়েছে।`);
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
              প্রোডাক্টস (তথ্য সংগ্রহ ও ক্যাটালগ)
            </h1>
            <span className="px-2.5 py-0.5 rounded-md bg-emerald-50 text-emerald-800 text-xs font-semibold border border-emerald-200">
              তথ্য ভিত্তিক • নো প্রাইস
            </span>
          </div>
          <p className="text-gray-500 text-sm mt-1">
            এই তালিকার পণ্যগুলি শুধুমাত্র খামারের জাত ও পরিচিতি পেজে প্রদর্শিত হয়। (কোনো মূল্য বা অর্ডার নেই)
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          {demoCount > 0 && (
            ...
            >
              ...
              <span>ডেমো ডাটা মুছুন ({demoCount})</span>
            </button>
          )}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-1">
                        ...
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-emerald-50 text-[#003F2D] border border-emerald-100 shrink-0">
                          {product.category}
                        </span>
                      </div>
                      <p className="text-xs text-gray-500 mt-1">
                        জাত: <span className="font-medium text-gray-700">{product.breed}</span> • {product.gender}
                      </p>
                      <p className="text-xs text-gray-600 mt-0.5 font-medium">
                        {product.age} {product.weight ? `• ${product.weight}` : ''}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer text-xs text-gray-600 font-medium">
                        <input
                          ...
                          onChange={(e) => toggleProductStock(product.id, e.target.checked)}
                      ...
                      <label className="flex items-center gap-2 cursor-pointer text-xs text-gray-600 font-medium">
                        <input
                          ...
                          onChange={(e) => toggleProductLimited(product.id, e.target.checked)}
                          className="sr-only peer"
                        />
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-200 text-gray-600 text-xs font-semibold">
                ...
                <th className="py-3.5 px-4">বয়স ও ওজন</th>
                ...
                filteredProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Category */}
                      <td className="py-3 px-4">
                        <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-emerald-50 text-[#003F2D] border border-emerald-100">
                          {product.category}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-4 text-xs">
                        <div className="text-gray-900 font-medium">{product.age}</div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 max-w-2xl w-full my-8 p-6 sm:p-8 relative">
            ...
            <div className="mb-6 pb-3 border-b border-gray-100">
              <h2 className="text-xl font-bold text-gray-900">
                {editingProduct ? 'প্রোডাক্ট তথ্য সম্পাদনা করুন' : 'নতুন তথ্য প্রোডাক্ট যোগ করুন'}
              </h2>
              <p className="text-xs text-gray-500 mt-1">
                জাত ও পরিচিতি বিবরণ পূরণ করুন (এই সেকশনে কোনো মূল্য বা অর্ডার বাটন থাকবে না)।
              </p>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="যেমন: দেশি বাছুর, ব্ল্যাক বেঙ্গল ছাগল"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="যেমন: দেশি, শাহিওয়াল, ফ্রিজিয়ান"
                    className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#003F2D] text-sm text-gray-900"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="যেমন: ১২০ কেজি, ১৮ কেজি"
                    className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#003F2D] text-sm text-gray-900"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="যেমন: পুরুষ / গাভী / খাসি / মাদি"
                    className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#003F2D] text-sm text-gray-900"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="যেমন: সুস্থ, নিয়মিত টিকা দেওয়া"
                    className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#003F2D] text-sm text-gray-900"
                  />
                </div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea
                    ...
                    placeholder="খামারে লালন-পালনের পদ্ধতি ও বৈশিষ্ট্য..."
                    className="w-full p-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#003F2D] text-sm text-gray-900"
                  />
                </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                >
                  {isSubmitting ? 'সংরক্ষণ হচ্ছে...' : editingProduct ? 'হালনাগাদ করুন' : 'প্রোডাক্ট যোগ করুন'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 max-w-sm w-full p-6 text-center">
            ...
            <p className="text-xs text-gray-500 mb-6">
              এটি ফায়ারবেস থেকে স্থায়ীভাবে মুছে যাবে।
            </p>
      ...
      {isDemoModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 max-w-sm w-full p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 text-base mb-1">
              সকল ডেমো ডাটা মুছে ফেলতে চান?
            </h3>
            <p className="text-xs text-gray-500 mb-6">
              মোট {demoCount} টি ডেমো আইটেম মুছে যাবে। আসল অ্যাডমিন ডাটা অক্ষত থাকবে।
            </p>
            <div className="flex items-center justify-center gap-3">
              <button
